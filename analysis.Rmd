...
## forroc$prob = prob
library(caret)
forroc.df <- data.frame(forroc)
forroc.df$D <- sapply(forroc.df$State,function(x){ifelse(grepl("TRUE", x), "true", "false")})
set.seed(112358)
fitControl <- trainControl(
    ...
    classProbs = T,
    savePredictions =T)
mod1 <- train(D ~ Freq + StrandAF, data = forroc.df,
                method = "glm",
                family=binomial(),
              trControl = fitControl)
forroc.df$prob <- predict(mod1, data=forroc.df, type=c("prob"))
forroc$prob <- forroc.df$prob[,"true"]
